...
// Rate limit config: 3 attempts per 5 minutes per token
...
export async function GET(request: Request) {
  try {
    ...
    // Get investor emails
    const investorEmails = verificationResult.campaign.startup.matches
      .flatMap((match: { investorEmails: any; }) => match.investorEmails)
      .map((ie: { email: any; }) => ie.email);
